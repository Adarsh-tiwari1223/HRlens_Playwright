"""
Director Page Object for HR Lens Portal Director Module (Under Masters).
...
class DirectorPage(BasePage):
    ...
    def navigate_to_directors(self):
        """Navigates to the Directors module page under Masters."""
        log_debug("Navigate", "Directors")
        if "/directors" not in self.page.url:
            # First try left sidebar Master menu -> Director
            master_menu = self.page.get_by_role("menuitem", name="Master")
            if master_menu.is_visible():
                master_menu.click()
            link = self.page.get_by_role("link", name="Directors", exact=False)
            if link.is_visible():
                link.click()
            else:
                self.page.goto(f"{settings.BASE_URL}/masters/director")
