...
def pending_leave(leave_response, leave_dates):
    ...
    assert my_leave is not None, f"Applied leave {start_date}->{end_date} not found"
    approver_name = my_leave.get("approvalManager", "")
    approver_key = settings.APPROVERS.get(approver_name)
    assert approver_key, f"Approver not found in APPROVERS map"
    ...
    assert matched is not None, f"Leave not found in pending queue"
...
def test_pending_leave_data_is_valid(pending_leave):
    ...
    assert "id" in leave
    assert leave.get("status") == "Pending"
    assert leave.get("fromDate")
    assert leave.get("toDate")
    assert leave.get("name")


@pytest.mark.api
def test_reject_leave_by_approver(pending_leave):
    leave, approver_key = pending_leave
    response = reject_leave(leave_id=leave["id"], reason="for the testing purpose only", approver=approver_key)
