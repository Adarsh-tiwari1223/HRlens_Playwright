...
class OfferLetterTemplatePage(BasePage):
    ...
    def add_section(self, section: dict, start_order: int = 1) -> tuple[bool, int]:
        ...
        # Fill title field (Playwright natively waits for it to become visible)
        title_field = self.page.locator(self.TITLE_INPUT).first
        title_field.click()
        try:
            # Clear field explicitly before filling to handle React state retention
            title_field.clear()
            title_field.fill(section["title"])
        except Exception:
            title_field.type(section["title"])
        ...
        try:
            ...
            content_field.type(section["content"])

        # Retry loop for Order
        current_order = start_order
        for _ in range(20):  # Maximum 20 retries
            # Set order
            order_field = self.page.get_by_label("Order").last
            order_field.click()
            try:
                order_field.clear()
                order_field.fill(str(current_order))
            except Exception:
                pass
            ...
            # Capture toast message to detect duplicates, order errors, or success
            try:
                ...
                # If the order is already taken, increment and retry saving
                if "sort order" in toast_text and "already exist" in toast_text:
                    print(f"[RETRY] Sort order {current_order} taken. Trying {current_order + 1}...")
                    current_order += 1
                    self.page.wait_for_timeout(1000) # Give toast a moment to fade
                    continue
                
                # If the section key is already taken, skip
                if "already exist" in toast_text:
                    print(f"[SKIP] Section '{section['key']}' already exists according to backend.")
                    # Close the modal using the user's exact provided locator
                    try:
            ...
            # If we reach here, it either succeeded or there was no toast error
            self.page.wait_for_load_state("networkidle")
    ...
    # ── UPDATE ─────────────────────────────────────────────────────────────────
    def update_section(self, section_key: str, new_title: str = None, new_content: str = None, enforce_order: int = None):
        """
